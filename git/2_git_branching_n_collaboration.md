...
* **ssh-keygen -t ed25519 -C "the email to which your Github is linked"** - команда для генерации SSH-ключа (первый вариант);  
* **ssh-keygen -t rsa -b 4096 -C "the email to which your Github is linked"** - в случае, если предыдущая команда не сработала (второй вариант);  
...
```clip < ~/.ssh/id_rsa.pub``` - если ключ генерировался с помощью второго варианта команды.   
...
Основная ветка по умолчанию называется '*main*'. В старых версиях `git` основная ветка называлась '*master*'. Чтобы переименовать с '*master*' на '*main*' используется следующая команда:  
```Bash
